...
def argument_parse():
    ...
    #parser.add_argument("-z", "--path_philips_sdk", help="path_philips_sdk", required="True")
...
def calc_patches_cord(list_binary_img,patch_level,svs_file,patch_dir,samp,patch_size,threshold_area_percent,toplevel):
    ...
    minx = 0
    miny = 0
    maxx = OSobj.level_dimensions[patch_level-1][0]
    maxy = OSobj.level_dimensions[patch_level-1][1]
    start_x = minx
    total_num_patches=0
    ...
    while start_x + patch_size < maxx:
        ...
        while start_y + patch_size < maxy:
            current_x=int((start_x*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
            current_y=int((start_y*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
            tmp_x = start_x  + 224
            tmp_y = start_y  + 224
            current_x_stop=int((tmp_x*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
            current_y_stop=int((tmp_y*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
...
def create_summary_img(patch_start_x_list,patch_stop_x_list,patch_start_y_list,patch_stop_y_list,samp,patch_dir,toplevel,patch_level,svs_file):
    ...
    for i in range(0,len(patch_stop_x_list),1):
        x1=int((patch_start_x_list[i]*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
        x2=int((patch_stop_x_list[i]*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
        y1=int((patch_start_y_list[i]*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
        y2=int((patch_stop_y_list[i]*OSobj.level_downsamples[patch_level-1])/OSobj.level_downsamples[toplevel])
...
def create_tfrecord(patch_start_x_list,patch_stop_x_list,patch_start_y_list,patch_stop_y_list,samp,patch_dir,patch_level,svs_file,toplevel,tf_output,patch_size,mut_type):
    ...
    for i in range(0,len(patch_start_x_list),1):
        x1 = patch_start_x_list[i]
        x2 = patch_stop_x_list[i]
        y1 = patch_start_y_list[i]
        y2 = patch_stop_y_list[i]
        img = OSobj.read_region((x1,y1), patch_level, (patch_size, patch_size))
        image_name=samp+"_x_"+str(x1)+"_"+str(x2)+"_y_"+str(y1)+"_"+str(y2)+".png"
        image_format="png"
        height = patch_size
        width = patch_size
        imgByteArr = io.BytesIO()
...
def main():
    ...
    create_tfrecord(patch_start_x_list,patch_stop_x_list,patch_start_y_list,patch_stop_y_list,samp,patch_dir,patch_level,svs_file,toplevel,tf_output,patch_size,mut_type)
